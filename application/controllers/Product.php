...
class Product extends CI_Controller {
                    ...
                    for ($i = 2; $i <= $arrayCount; $i++) {
                       ...
                       $error_data[] = array('category' => $category, 'brand_name' => $brand_name,'model_number'=>$model_number,'menu_name' => $menu_name);
                        $error_r += 1 ; 
                            ...
                            array("id"), "tbl_masterdata", array("category"=>$category,"brand_name"=>$brand_name,'model_number'=>$model_number,"menu_name"=>$menu_name,'segment' => $segment)
                            );
    ...
    public function delete_master3_all(){
    ...
    // $id = $this->uri->segment(3);  
     ...
     $data['main']='Product/journey_plan';
     $this->load->view('common/template',$data);        
    }
     ...
     public function edit_outlet_master(){
     $id = $this->uri->segment(3);  
     $data['edit_product'] = $this->Common_model->get_run("SELECT * FROM `tbl_outlet_master` where id = '$id'"); 
     $data['main']='Product/edit_outlet_master';
     $this->load->view('common/template',$data);        
    }
     public function update_outlet_master(){
         $id = $this->uri->segment(3);  
        $tbl_product   = 'tbl_outlet_master';
        ...
        if($result > 0){
            $this->session->set_flashdata('msg','Outlet master Add Successfully');
             redirect(base_url('Product/outlet_master'));
        }
        else{
            $this->session->set_flashdata('error_msg','Something Went Wrong!!! Try Again');
            redirect(base_url('Product/edit_outlet_master'));
        }           
    }
  ...
  if(!empty($invoice_data_val)){
    ...
    function images_list(){
       ...
       // $data['duration'] = $duration = isset($_GET['duration']) ? $_GET['duration'] : "";
        ...
        $con .= " and (`si.id` IS NOT NULL OR `ddt.id` IS NOT NULL OR `st.id` IS NOT NULL OR `sfc.id` IS NOT NULL OR `ms.id` IS NOT NULL OR `tt.id` IS NOT NULL) ";
       ...
       if(!empty($filter)){
            ...
            if(!empty($create_date_range_filter)){
                $con .= " and (DATE(new_date) >= '".$create_date_range_filter['from_date']."' AND DATE(new_date) <= '".$create_date_range_filter['to_date']."') ";
            }
        }
        
        $select = "`om`.`id` as main_id,`om`.`form_type` as form_type,si.id as si_main_id,ddt.id as ddt_main_id,st.id as st_main_id,sfc.id as sfc_main_id,ms.id as ms_main_id,tt.id as tt_main_id, om.outlet_name,om.account_name,om.region,om.user,om.new_date,st.image_array as sellout_report,sfc.add_more_array as special_fixture_count,ms.add_more_array as market_sensing,si.add_more_array as store_image,ddt.add_more_array as display_and_deployment_tracker,tt.add_more_array as training_tracker";
       ...
       // }
        ...
        $table = 'image_list_data as om';
        // $join = array(
        //          array("table" => "sellout AS st", "relation" => "om.outlet_name = st.outlet_name and om.new_date = DATE(st.date)", "type" => "LEFT"),
        //          array("table" => "special_fixture_count AS sfc", "relation" => "sfc.outlet_name = om.outlet_name and om.new_date = DATE(sfc.date)", "type" => "LEFT"),
        //          array("table" => "market_sensing AS ms", "relation" => "om.outlet_name = ms.outlet_name and om.new_date = DATE(ms.date)", "type" => "LEFT"),
        //          array("table" => "300_store_image AS si", "relation" => "om.outlet_name = si.outlet_name and om.new_date = DATE(si.date)", "type" => "LEFT"),
        //          array("table" => "display_and_deployment_tracker AS ddt", "relation" => "om.outlet_name = ddt.outlet_name and om.new_date = DATE(ddt.date)", "type" => "LEFT"),
        //          array("table" => "training_tracker AS tt", "relation" => "om.outlet_name = tt.outlet_name and om.new_date = DATE(tt.date)", "type" => "LEFT"),
        //     );
        $join = array(
                 array("table" => "sellout AS st", "relation" => "om.outlet_name = st.outlet_name and om.new_date = DATE(st.date) and om.user_id = st.user_id and om.form_type ='sell_out_report' ", "type" => "LEFT"),
                 array("table" => "special_fixture_count AS sfc", "relation" => "sfc.outlet_name = om.outlet_name and om.new_date = DATE(sfc.date) and om.user_id = sfc.user_id and om.form_type ='special_fixture_count' ", "type" => "LEFT"),
                 array("table" => "market_sensing AS ms", "relation" => "om.outlet_name = ms.outlet_name and om.new_date = DATE(ms.date) and om.user_id = ms.user_id and om.form_type ='market_sensing' ", "type" => "LEFT"),
                 array("table" => "300_store_image AS si", "relation" => "om.outlet_name = si.outlet_name and om.new_date = DATE(si.date) and om.user_id = si.user_id and om.form_type ='300_store_image' ", "type" => "LEFT"),
                 array("table" => "display_and_deployment_tracker AS ddt", "relation" => "om.outlet_name = ddt.outlet_name and om.new_date = DATE(ddt.date) and om.user_id = ddt.user_id and om.form_type ='display_and_deployment_tracker' ", "type" => "LEFT"),
                 array("table" => "training_tracker AS tt", "relation" => "om.outlet_name = tt.outlet_name and om.new_date = DATE(tt.date) and om.user_id = tt.user_id and om.form_type ='training_tracker' ", "type" => "LEFT"),
            );
        ...
        //$invoice_data_val = $this->Common_model->getPaginationList($start, $table, $select, $con, $orderBy, $limit,$join,null,TRUE,'om.outlet_name,om.new_date');
         $invoice_data_val = $this->Common_model->getPaginationList($start, $table, $select, $con, $orderBy, $limit,$join,null,TRUE,null);
        // echo $this->db->last_query();
        //    echo '<pre>';print_r($invoice_data_val);
        //  exit;
        $invoice_total = $this->Common_model->getData($table, $select, $con, $orderBy, null, $join,null,TRUE,null);
       // echo $this->db->last_query();
        $count_invoice = 0;
        if(!empty($invoice_total)){
            $count_invoice = count($invoice_total);
    ...
    function show_images(){
        ...
        }elseif($diff == 'sell_out_report'){
            $select = "*";
            $table = "sellout";
            $con = array('id' => $id);
            $result = $this->Common_model->getData($table, $select, $con, NULL, null, null,null,false);
            $data['result']=$result;
            $data['main']='Product/image_sellout_detail';
        
        }elseif($diff == 'special_fixture_count'){
            $select = "*";
            $table = "special_fixture_count";
            $con = array('id' => $id);
            $result = $this->Common_model->getData($table, $select, $con, NULL, null, null,null,false);
            $data['result']=$result;
            $data['main']='Product/default_show_images';
        
        }elseif($diff == 'market_sensing'){
            $select = "*";
            $table = "market_sensing";
            $con = array('id' => $id);
            $result = $this->Common_model->getData($table, $select, $con, NULL, null, null,null,false);
    ...
    function download_all_file(){
       ...
       // }
        ...
        //     );
        // $join = array(
        //          array("table" => "sellout AS st", "relation" => "om.outlet_name = st.outlet_name and om.new_date = DATE(st.date) and om.user_id = st.user_id ", "type" => "LEFT"),
        //          array("table" => "special_fixture_count AS sfc", "relation" => "sfc.outlet_name = om.outlet_name and om.new_date = DATE(sfc.date) and om.user_id = sfc.user_id ", "type" => "LEFT"),
        //          array("table" => "market_sensing AS ms", "relation" => "om.outlet_name = ms.outlet_name and om.new_date = DATE(ms.date) and om.user_id = ms.user_id", "type" => "LEFT"),
        //          array("table" => "300_store_image AS si", "relation" => "om.outlet_name = si.outlet_name and om.new_date = DATE(si.date) and om.user_id = si.user_id", "type" => "LEFT"),
        //          array("table" => "display_and_deployment_tracker AS ddt", "relation" => "om.outlet_name = ddt.outlet_name and om.new_date = DATE(ddt.date) and om.user_id = ddt.user_id", "type" => "LEFT"),
        //          array("table" => "training_tracker AS tt", "relation" => "om.outlet_name = tt.outlet_name and om.new_date = DATE(tt.date) and om.user_id = tt.user_id", "type" => "LEFT"),
        //     );
        ...
        $row = $this->Common_model->getData($table, $select, $con, $orderBy, null, $join,null,false,null);
